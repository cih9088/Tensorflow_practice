...
class ALI(object):
    ...
    def get_loss(self, D_p_logits, D_n_logits):
        D_loss = tf.reduce_mean(tf.nn.softplus(-D_p_logits)
                + tf.nn.softplus(-D_n_logits) + D_n_logits)
        G_loss = tf.reduce_mean(tf.nn.softplus(-D_p_logits) + D_p_logits
                + tf.nn.softplus(-D_n_logits))
        ...
        for epoch in xrange(config.max_epoch):
            ...
            with tqdm(total=total_batch) as pbar:
                for batch, label in self.data_set._iter(self.batch_size * self.n_gpu, 'train'):
                    ...
                    if config.monitor:
                        gen_imgs = self.sess.run(self.tower_reconst_x[0])
