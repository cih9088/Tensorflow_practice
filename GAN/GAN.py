...
class GAN(object):
        ...
        for i in xrange(self.n_gpu):
            with tf.device('/gpu:%d' % i):
                with tf.name_scope('Tower_%d' % (i)) as scope:
                    ...
                    # Construct the model
                    z, G, D_p, D_p_logits, D_n, D_n_logits = \
    ...
    def build_model_per_gpu(self, inputs):
        ...
        D_p, D_p_logits = self.discriminator(inputs, reuse=False)
        G = self.generator(z)
        D_n, D_n_logits = self.discriminator(G, reuse=True)

        return z, G, D_p, D_p_logits, D_n, D_n_logits
